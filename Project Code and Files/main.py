...
from emo import AttendanceApp as CAI
...
class FRS:
    def __init__(self,root):
        ...
        # images
        imgbg=Image.open("images/bg.jpg")
        ...
        fl1=Label(bg,text="Smart Student Attendance System",font=("times new roman",35,"bold"),bg="navy",fg="white")
...
# college logo /*
        ...
        b2.place(x=480,y=180,width=250,height=220)

        b2_1=Button(bg,text="Face Detector",cursor="hand2",command=self.face_recong,font=("times new roman",20,"bold"),bg="navy",fg="white")
        b2_1.place(x=480,y=380,width=250,height=40)

        # button rfid
        imgr=Image.open("images/11.jpg")
        imgr=imgr.resize((250,220),Image.LANCZOS)
        self.col3=ImageTk.PhotoImage(imgr)

        br=Button(bg,image=self.col3,cursor="hand2",command=self.open_rfid)
        br.place(x=800,y=180,width=250,height=220)

        br_1=Button(bg,text="RFID",command=self.open_rfid,cursor="hand2",font=("times new roman",20,"bold"),bg="navy",fg="white")
        br_1.place(x=800,y=380,width=250,height=40)

        # button3
        img3=Image.open("images/5.jpg")
        img3=img3.resize((250,220),Image.LANCZOS)
        self.col4=ImageTk.PhotoImage(img3)

        b3=Button(bg,image=self.col4,cursor="hand2",command=self.att_det)
        b3.place(x=1120,y=180,width=250,height=220)

        b3_1=Button(bg,text="Student Attendance",cursor="hand2",command=self.att_det,font=("times new roman",20,"bold"),bg="navy",fg="white")
        b3_1.place(x=1120,y=380,width=250,height=40)

        # button5
        img5=Image.open("images/4.jpg")
        ...
        b6.place(x=480,y=465,width=250,height=220)

        b6_1=Button(bg,text="Student Data",cursor="hand2",command=self.open_img,font=("times new roman",20,"bold"),bg="navy",fg="white")
        b6_1.place(x=480,y=665,width=250,height=40)

        # button7
        img7=Image.open("images/2.jpg")
        img7=img7.resize((250,220),Image.LANCZOS)
        self.col8=ImageTk.PhotoImage(img7)

        b7=Button(bg,image=self.col8,cursor="hand2",command=self.abus)
        b7.place(x=1120,y=465,width=250,height=220)

        b7_1=Button(bg,text="About US",cursor="hand2",command=self.abus,font=("times new roman",20,"bold"),bg="navy",fg="white")
        b7_1.place(x=1120,y=665,width=250,height=40)

        # button ai
        imgai=Image.open("images/12.jpg")
        imgai=imgai.resize((250,220),Image.LANCZOS)
        self.col7=ImageTk.PhotoImage(imgai)

        b8=Button(bg,image=self.col7,cursor="hand2",command=self.ai_comp)
        b8.place(x=800,y=465,width=250,height=220)

        b8_1=Button(bg,text="AI Chat Bot",command=self.ai_comp,cursor="hand2",font=("times new roman",20,"bold"),bg="navy",fg="white")
        b8_1.place(x=800,y=665,width=250,height=40)
        
        b9=Button(bg,text="EXIT",cursor="hand2",command=self.iexit,font=("times new roman",20,"bold"),bg="navy",fg="white")
        b9.place(x=650,y=740,width=240,height=40)
    ...
    def face_det(self): #face detection
        self.new_window=Toplevel(self.root)
        self.app=FAE(self.new_window)

    def ai_comp(self): #ai and compare files
        self.new_window=Toplevel(self.root)
        self.app=CAI(self.new_window)

    def open_img(self): #open img folder
        os.startfile("data")

    def abus(self): #about us
        self.new_window=Toplevel(self.root)
    ...
    def open_rfid(self):
        try:
            file_path = "PLX-DAQ-v2.11.xlsm"  # Update the path if necessary
            os.startfile(file_path)  # Opens the file with the default application
        except Exception as e:
            messagebox.showerror("Error", f"Unable to open RFID file: {str(e)}", parent=self.root)
    ...
    def mark_att(self, i, j, k, n):  # Attendance
        file_path = f"Attendance/{self.filename}"
        ...
        with open(file_path, "a", newline="\n") as f:
            ...
            if not file_exists:
                f.write("ID,Name,USN,Dept,Time,Date,Status\n")
            f.writelines(f"{n},{j},{i},{k},{d2},{d1},Present\n")

    # def mark_att(self,i,j,k,n): #attendence
    #     with open(f"Attendance/{self.filename}", "a", newline="\n") as f:
    #         now = datetime.now()
    #         d1 = now.strftime("%d/%m/%Y")
    #         d2 = now.strftime("%H:%M:%S")
    ...
    def face_recong(self):
        def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            gray_image = preprocess_image(img)  # Apply preprocessing

            # Tune the scaleFactor and minNeighbors
            features = classifier.detectMultiScale(gray_image, scaleFactor=1.05, minNeighbors=5)
        ...
        clf = cv2.face.LBPHFaceRecognizer_create(radius=2, neighbors=16, grid_x=8, grid_y=8)
